...
**同步 IO**：会导致请求进程阻塞，知道 I/O 操作完成。例子：CPU 在执行代码的时候，一旦遇到 IO 操作，比如读写文件，发送网络数据时，就需要等待 IO 操作的完成，才能继续进行下一步操作，这种情况就是同步 IO 。
...
同步：体现在消息通知机制，这个线程仍然要定时的读取 stream，判断数据有没有准备好， client 采用循环的方式去取，这样大部分 CPU 资源会被浪费。

非阻塞：体现在，这个线程还可以去干别的，不需要一直等着。
